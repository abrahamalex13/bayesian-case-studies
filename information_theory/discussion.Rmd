...
Let's assess data distribution _drift_ between a test set and historical set.
Said another way, how dissimilar is the test data distribution from historical?
Conclusions depend on the test dataset's observation count--
with smaller data samples, the drift measure (KL Divergence) should vary more widely. 
So we'll examine different choices of test dataset size,
ultimately _tuning_ the decision for our use case.

Let's study real data made available by Carvana.
...
Let's examine drift in Make, specifically.

## Manually Compare Distributions from Entire Data

Let's first eyeball the extent of drift, using the entire data. 
This manual assessment will help tune an automated method,
_useful for live future monitoring_.

Visually, the probability distribution appears similar, test versus historical:

```{r}
  knitr::include_graphics(paste0(DIR_PLOTS, "/drift_viz_overall.png"))
```

In the absence of deeper domain knowledge to suggest otherwise,
let's propose the ground truth of no meaningful data drift.
The same assessment should arise from a properly-tuned automated method.

## Statistically, Test Set Has Drifted (As Expected)

For automated data drift detection to perceive no drift,
a test event must compare similarly to other no-drift scenarios. 
Even under no _population_ data drift,
data _samples_ will not have identical distributions.

We can simulate no-drift scenarios from the historical set--
by repeated data sampling, with specific sample size.
Using each scenario's distribution drift measure 
(KL Divergence, described farther down), 
we can review extreme possible scenarios, 
and assign relative score to a new test event.

It turns out that a test set of ~49,000 observations
would practically never appear drifted to the entire data extent.
When a test set of ~49,000 observations draws direct from historical,
the apparent drift will rarely appear more extreme than below:

```{r}
  knitr::include_graphics(paste0(DIR_PLOTS, "/drift_viz_boot_naive.png"))
```

We should not expect real data to reflect zero drift -- 
the generating conditions are dynamic and likely time-varying.
